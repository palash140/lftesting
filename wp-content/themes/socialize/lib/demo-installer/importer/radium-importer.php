 ...
 */
 ...
 if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
		 ...
		 * @var object
		 */
		public $theme_options_framework = 'socialize'; //supports radium framework and option tree
	    ...
	    public function __construct() {
			...
			//add_action( 'widgets_init', array( $this, 'ghostpool_custom_sidebars' ) ); // MOD
		...
		/*public function ghostpool_custom_sidebars() {	
			if ( get_option( 'ghostpool_theme_activated' ) !== '1' ) {
				...
				update_option( 'ghostpool_theme_activated', '1' );
			}
    	...
    	public function intro_html() {
			...
			<p><?php esc_html_e( 'Importing demo data is the easiest way to setup your theme. It will allow you to quickly edit everything seen in the theme demo instead of creating everything from scratch. When you import the demo data the following things will happen:', 'socialize' ); ?></p>

			<ul class="ul-square">
				<li><?php esc_html_e( 'No existing data will be deleted or modified.', 'socialize' ); ?></li>
				<li><?php esc_html_e( 'No WordPress settings will be modified.', 'socialize' ); ?></li>
				<li><?php esc_html_e( 'Posts, pages, categories, media, widgets and menus will get imported.', 'socialize' ); ?></li>
			</ul>
		  
			<div class="update-nag">
				<p><strong><?php wp_kses( _e( '*** PLEASE READ BEFORE IMPORTING THE DEMO CONTENT ***<ol><li>You will see grey placeholder images instead of the images seen on the theme demo website due to copyright laws.</li><li>Please active BuddyPress and bbPress before importing the demo content. BuddyPress Activity, Groups and Members seen in the demo will NOT be imported. You can import dummy BuddyPress data using the <a href="https://wordpress.org/plugins/bp-default-data/" target="_blank">BuddyPress Default Data</a> plugin.</li></ol>', 'socialize' ), array( 'a' => array( 'href' => array(), 'target' => array() ) ) ); ?></strong></p>
			</div>
	    ...
	    public function demo_installer() {
			...
			if( !empty($this->imported_demos ) ) {
				...
				$button_text = esc_html__('Import Again', 'socialize' );

			} else {

				$button_text = esc_html__('Import Demo Data', 'socialize' );
			...
			<div class="wrap">
				...
				<h2><?php esc_html_e( 'Import Demo Data' , 'socialize' ); ?></h2>
						...
						<script>
							jQuery( function ( $ ) {
								$( '.js-one-click-import-form' ).on( 'submit', function () {
									$( this ).append( '<br/><p><span class="spinner" style="visibility: visible; display: block; float: left; margin: 0 5px 0 0;"></span> <strong><?php esc_html_e( "Importing now, this can take a few minutes", "socialize" ); ?></strong></p>' );
							...
							<?php if ( $this->flag_as_imported['content'] && $this->flag_as_imported['widgets'] ) { ?>
								<div class="radium-importer-success">
									<?php if ( ! empty( $this->imported_demos ) ) { ?>
										<p><?php esc_html_e( 'You have successfully imported demo data that was not imported last time.', 'socialize' ); ?></p>
									<?php } else { ?>
										<p><?php esc_html_e( 'You have successfully imported the demo data.', 'socialize' ); ?></p>
									<?php } ?>	
								</div>
							<?php } ?>
	    ...
	    public function set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				if( $this->theme_options_framework == 'socialize' ) {
				...
				if ( !empty( $data ) || is_array( $data ) ) {
	      		...
	      		wp_die(
      				esc_html__( 'Theme options Import file could not be found. Please try again.', 'socialize' ),
      				'',
	    ...
	    function process_widget_import_file( $file ) {
	    	...
	    	if ( ! file_exists( $file ) ) {
	    		wp_die(
	    			esc_html__( 'Widget Import file could not be found. Please try again.', 'socialize' ),
	    			'',
	    			array( 'back_link' => true )
	    		);
	    	}
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	    			...
	    			$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'socialize' );
	    		}
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	    				...
	    				$widget_message = esc_html__( 'Site does not support widget', 'socialize' ); // explain why widget not imported
	    			}
	    			...
	    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	    				...
	    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
	    					...
	    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	    						...
	    						$widget_message = esc_html__( 'Widget already exists', 'socialize' ); // explain why widget not imported
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = esc_html__( 'Imported', 'socialize' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = esc_html__( 'Imported to Inactive', 'socialize' );
	    				}
	    			...
	    			//$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'socialize' ); // show "No Title" if widget instance is untitled
