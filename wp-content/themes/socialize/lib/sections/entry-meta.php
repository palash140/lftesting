<?php if ( ( isset( $GLOBALS['ghostpool_meta_author'] ) && $GLOBALS['ghostpool_meta_author'] == '1' ) OR ( isset( $GLOBALS['ghostpool_meta_date'] ) && $GLOBALS['ghostpool_meta_date'] == '1' ) OR ( isset( $GLOBALS['ghostpool_meta_comment_count'] ) && $GLOBALS['ghostpool_meta_comment_count'] == '1' ) OR ( isset( $GLOBALS['ghostpool_meta_views'] ) && $GLOBALS['ghostpool_meta_views'] == '1' ) ) { ?>
	...
	<div class="gp-entry-meta">
		...
		<?php if ( isset( $GLOBALS['ghostpool_meta_author'] ) && $GLOBALS['ghostpool_meta_author'] == '1' ) { ?>
			<span class="gp-post-meta gp-meta-avatar"><?php echo apply_filters( 'gp_author_avatar', '<a href="' . get_author_posts_url( $post->post_author ) . '">' . get_avatar( get_the_author_meta( 'ID', $post->post_author ), 50 ) . '</a>', $post ); ?></span>
		<?php } ?>
		...
		<span class="gp-post-meta-rows">	
			...
			<span class="gp-post-meta-row-1">
				...
				<?php if ( isset( $GLOBALS['ghostpool_meta_author'] ) && $GLOBALS['ghostpool_meta_author'] == '1' ) { ?>
					<span class="gp-post-meta gp-meta-author"><?php esc_html_e( 'By', 'socialize' ); ?> <?php echo apply_filters( 'gp_author_url', '<a href="' . esc_url( get_author_posts_url( $post->post_author ) ) . '">' . get_the_author_meta( 'display_name', $post->post_author ) . '</a>', $post ); ?></span>
				<?php } ?>
